...
class Discriminator:
    ...
    def __init__(self):
        self.models = [
            keras.models.load_model(f"discriminator_models/discriminator_{i+1}.h5")
            for i in range(5)
        ]
    ...
    def train(self, generated_images):
        ...
        real_images = REAL_IMAGES.sample(frac=1).reset_index(drop=True)[
            : len(generated_images)
        ]  # Random subset of real images to train on
        combined_images = pd.concat(
            [real_images, generated_images], ignore_index=True
        )  # Combine the real and fake images
        combined_images = combined_images.sample(frac=1).reset_index(
            drop=True
        )  # Shuffle
