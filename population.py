...
class Population:
    def __init__(self, size=20, doping_size=10):
    ...
    def _select_top_n(self, n_parents=10, offsprings=10):
        ...
        for i in range(1, offsprings, 2):
            ...
            offspring_a = Image(chromosome=np.concatenate([parents[i-1].chromosome[:split_point], parents[i].chromosome[split_point:]]))
            offspring_b = Image(chromosome=np.concatenate([parents[i].chromosome[:split_point], parents[i-1].chromosome[split_point:]]))
