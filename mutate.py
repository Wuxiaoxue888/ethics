...
def mutate(offspring):
    """
    Mutates given offspring with probability MUTATION_PROBABILITY by randomly selecting a set of mutation functions.
    :param offspring: Offspring that will be mutated
    """
    if random.random() < MUTATION_PROBABILITY:
        return

    x = [_mutate_1, _mutate_2, _mutate_3]
    p = random.sample(x, k=random.randint(1, len(x)))

    for mutation_func in p:
        mutation_func(offspring)
...
def _mutate_2(offspring):
    ...
    if offspring[x][y] < 0.4:
        ...
        # call strengthen line, to turn pixels black
